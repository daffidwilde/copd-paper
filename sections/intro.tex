...
\input{tex/nspells}spells. The dataset is made up of instances each describing a
patient spell with the following attributes:
\begin{itemize}
    \item Personal identifiers, i.e.\ patient and spell ID numbers, and age.
    \item Admission/discharge dates and approximate times.
    \item A number of attributes summarising the clinical path of the spell
        including admission/discharge methods, and the number of episodes,
        consultants and wards in the spell.
    \item Diagnosis (ICD) and primary procedure (HRG) codes from each episode.
    ...
    \item Rank under the 2019 Welsh Index of Multiple Deprivation (WIMD)
        indicating relative deprivation of the area the patient lives in.
\end{itemize}

In addition to the above, the following attributes were engineered:
\begin{itemize}
    \item Age and overall spell cost data were linked to approximately half of
        the spells in the dataset from another administrative dataset provided
        by the Cwm Taf Morgannwg UHB.\
    \item To better capture the kinds of conditions present in each spell, the
        presenting ICD codes were generalised to their categories according to
        NHS documentation and counts for each category were attached to the
        spell.
    \item The number of COPD-related admissions in the last twelve months based
        on the patient ID number.
\end{itemize}

For the segmentation, a variant of the \(k\)-means algorithm that allows for the
clustering of mixed-type data was used to partition the spells in the dataset.
This algorithm, presented in~\cite{Huang1998} along with the categoric-only
\(k\)-modes algorithm, is called \(k\)-prototypes and uses \(k\)-means on the
numeric data and \(k\)-modes on the categoric data simultaneously. The
attributes included in the clustering encompass both utilisation metrics and
clinical attributes of the patients in the spell. They were as follows: the
summative clinical path attributes, the CCI contributions and condition
indicators, the WIMD rank, length of stay (LOS), COPD intervention status, and
the engineered attributes (not including age and costs due to lack of coverage).
...
method' for determining a number of clusters. This revealed an optimal value
for \(k\) of 4 but both 3 and 5 clusters were considered. Each case was
...
\begin{table}
    ...
    \resizebox{\textwidth}{!}{%
        ...
        A summary of patient-level clinical attributes and disease prevalence by
        cluster and for the population.
    }\label{tab:summary}
\end{table}
