...
critical specifications are that it must handle mixed-type data, and that it
should be interpretable by stakeholders. The authors of~\cite{Jahangirian2015}
...
standard deviations for the numeric attributes. The analysis that informed the
clustering in this work found that this value for \(\gamma\) provided a useful
...
from two to ten. This range is based on what may be considered feasibly
...
Although the dataset is confidential, a synthetic analogue which illustrates the
clustering has been archived under~\texttt{DOI:10.5281/zenodo.3908167}. A
summary of the dataset and its clustering is provided in
Table~\ref{tab:summary}. Note that a negative length of stay indicates that the
patient had passed away prior to arriving at the hospital; these spells have
been omitted from further analysis. This table separates each cluster and the
overall dataset (referred to as the population). From this table, helpful
insights can be gained about the segments identified by the clustering. For
instance, the needs of each cluster can be summarised succinctly:
